...
void Communicator::connectToServer()
{
    ...
    QString serverIp = settings->value(cst::settingsNames.lines[5]).toString();
    qDebug() << "serverIp: " << serverIp;
    if (settings->getIsConnectedToSettings())
        settings->sendDebugMessage("serverIp is: " + serverIp);
...
{
    //settings->sendDebugMessage("connected to the server");
...
void Communicator::readyRead()
{
    ...
    if (settings->getIsConnectedToSettings())
        settings->sendDebugMessage("from server: " + msg);
}
...
void Communicator::sendComPortRequest()
{
    ...
    qDebug() << data;
    //settings->sendDebugMessage("sending Communication Port Request: " + QString(data));
    socket->write(data);
}
...
void Communicator::sendAccessPriceRequest(QString cardNumber, double price, int time)
{
    ...
    qDebug() << data;
    //settings->sendDebugMessage("sending Access With Price Request: " + QString(data));
    socket->write(data);
}
