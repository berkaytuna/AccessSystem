...
bool Reader::init()
{
    ...
    for (const QSerialPortInfo &info : infos) {
        ...
        if (vid == cst::LF.vid && pid == cst::LF.pid) {
            ...
            else if (description == "USB Serial") {
                qDebug() << "found Arduino!";
                serialPort = new QSerialPort;
                readerNames.push_back("Arduino");
                addReader(info, serialPort);
                connect(serialPort, &QSerialPort::readyRead, this, &Reader::readyRead);
            }
        }
...
void Reader::readyRead()
{
    ...
    QString rawCardNumber = cardNumber;
    ...
    if (readerName == cst::LF.friendlyName) {
        ...
        if (settings->isCheckedSingleArrays[9])
            turnBits(cardNumber);
    }
        ...
        if (settings->isCheckedSingleArrays[7])
            turnBytes(cardNumber);
        if (settings->isCheckedSingleArrays[10])
            turnBits(cardNumber);
    }
    else if (readerName == cst::Arduino.friendlyName) {
        removeChars(cardNumber, settings->isCheckedArrays[2]);
        if (settings->isCheckedSingleArrays[2])
            removeZeroFirst(cardNumber);
        if (settings->isCheckedSingleArrays[5])
            removeZeroInside(cardNumber);
        if (settings->isCheckedSingleArrays[8])
            turnBytes(cardNumber);
        if (settings->isCheckedSingleArrays[11])
            turnBits(cardNumber);
    }

    qDebug() << "modified: " + cardNumber;
    if (settings->getIsConnectedToSettings())
        settings->sendDebugMessage("from: " + readerName + ", raw card number: " + rawCardNumber + ", modified: " + cardNumber);
