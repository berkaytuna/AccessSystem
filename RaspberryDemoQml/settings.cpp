...
{
    QString s;
    s = currentD() ? cst::settingsNames.lines[3] : cst::settingsNames.lines[1];
    setNum(value(s).toInt());
    s = currentD() ? cst::settingsNames.lines[4] : cst::settingsNames.lines[2];
...
void Settings::sendDebugMessage(const QString &msg)
{
